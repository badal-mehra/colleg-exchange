...
const Auth = () => {
  ...
  // FIX: Separate password visibility states
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);

  // FIX: Safe default value
  const [selectedUniversity, setSelectedUniversity] = useState(
    UNIVERSITY_OPTIONS?.[0]?.value || ""
  );

  // ✅ FIX: Prevent white screen during auth state resolution
  if (loading) return null;

  // ✅ FIX: Only redirect AFTER loading is done
  if (user) {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Optional: Handle post-signup DB logic here
      ...
      >
        <Card className="w-full shadow-2xl bg-white/90 backdrop-blur-sm">
          <CardHeader className="text-center relative">
            ...
            <CardDescription>Sign in or create your LPU student account</CardDescription>
          </CardHeader>
              ...
              <TabsContent value="signup" className="space-y-4">
                <form onSubmit={handleSignUp} className="space-y-4">
                  ...
                  <div className="flex items-start space-x-2">
                    <input type="checkbox" id="terms" name="terms" required className="mt-1 h-4 w-4 rounded" />
